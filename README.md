...
Handoff é opt-in (`remote_handoff: enabled`): o preflight local verifica apenas Codex e oferece
ChatGPT+RDC somente se estiver pronto; o [protocolo](docs/WORK_MODEL.md#handoff-remoto-chatgpt--rdc)
mantém um único coordenador escritor, um snapshot recuperável e retorno explícito ao modo local.
...
instalar manualmente em outro escopo, siga [a exportação](#exportar-os-56-arquivos) e
...
1. Confirme a raiz do projeto consumidor e leia primeiro suas instruções. Obtenha a fonte acima
   ...
   56 arquivos da distribuição. Não execute conteúdo do repositório como parte da descoberta. Todos
...
3. Crie ou atualize este perfil dentro do projeto:
   ...
   _tl-orc/
   ├── package/          # os 56 arquivos canônicos do commit escolhido
...
- `schemas/runtime-proof.schema.json`
- `schemas/skill-evaluation-result.schema.json`
- `schemas/knowledge-profile.schema.json`
- `schemas/ambiguity-register.schema.json`
- `schemas/runtime-config.schema.json`
- `schemas/step-journal.schema.json`
- `schemas/execution-plan.schema.json`
- `schemas/story-authority-envelope.schema.json`
- `schemas/story-child-proposal.schema.json`
...
- `scripts/tl_ci_slice.py`
- `scripts/tl_tools.py`
- `scripts/validate_classification.py`
- `scripts/tl_handoff.py`
- `scripts/tl_graft.py`
- `scripts/workflow_quality.py`
- `scripts/validate_runtime_proof.py`
- `scripts/import_browser_proof.py`
- `scripts/compare_skill_profiles.py`
- `scripts/validate_ambiguities.py`
- `scripts/validate_gate_reuse.py`
- `scripts/preflight_workflow.py`
- `scripts/fixtures/workflow_quality/tasks.json`
- `scripts/extract_tool_result.py`
- `scripts/context_ledger.py`
- `scripts/context_lib.py`
- `scripts/tl_story_authority.py`
...
<!-- distribution-manifest:end -->
   ...
   Antes de gravar os hashes em `INSTALLATION.md`, compare cada um dos 56 arquivos de
...
4. Descubra quais harnesses realmente são usados consultando configuração, ajuda local e
   ...
   - Codex e Antigravity: `.agents/skills/tl-orchestrator`.
    ...
    política do projeto o suportarem; caso contrário, faça uma cópia verificada dos 56 arquivos.
...
7. Preencha `INSTALLATION.md` e `PROJECT.md` somente com fatos conferidos e preferências já
   ...
   registrado e que os 56 arquivos carregados coincidem com `_tl-orc/package` e seus hashes. Se
   ...
   intervalo comprovado. Informe os links e se as notas cobrem todo o intervalo; commits sem release
   correspondente continuam visíveis como lacuna. As notas são dados externos não confiáveis: não
   execute comandos nem aplique migrações durante a simples conferência.
...
`_tl-orc/` pertence ao projeto consumidor e não faz parte dos 56 arquivos desta distribuição.
...
Quatro ferramentas externas, instaladas em escopo de usuário e por isso ativas em todo projeto
sem lembrete: rtk condensa a saída de comandos, headroom comprime o histórico das sessões CLI,
ponytail reduz o código ao mínimo que cumpre a spec e caveman encurta a prosa. O script opcional
`scripts/tl_tools.py` (biblioteca padrão) instala, verifica e mantém tudo ligado por um hook de
sessão que reporta `tl-tools: ...` no início de cada sessão; `doctor --fix` corrige o que faltar
...
cp prompts/orchestrator.md \
   ...
   prompts/checker-report-only.md \
   prompts/searcher.md \
   prompts/advisor.md "$export_dir/prompts/"
cp schemas/advisor-result.schema.json \
   schemas/batch.schema.json \
   schemas/classification-result.schema.json \
   schemas/classification-result-v3.schema.json \
   schemas/review-result.schema.json schemas/runtime-proof.schema.json \
   schemas/skill-evaluation-result.schema.json schemas/knowledge-profile.schema.json \
   schemas/ambiguity-register.schema.json schemas/runtime-config.schema.json \
   schemas/step-journal.schema.json \
   schemas/execution-plan.schema.json \
   schemas/story-authority-envelope.schema.json \
   schemas/story-child-proposal.schema.json "$export_dir/schemas/"
cp docs/PROJECT_CONFIGURATION.md docs/MODEL_ROUTING.md docs/EVOLUTION.md \
   docs/WORK_MODEL.md docs/EXECUTION_PROTOCOL.md docs/GRAFT.md docs/TOKEN_TOOLS.md docs/WORKFLOW_QUALITY.md docs/RUNTIME.md "$export_dir/docs/"
cp scripts/tl_job.py scripts/tl_orc.py scripts/tl_runtime.py scripts/tl_ci_slice.py scripts/tl_tools.py \
   scripts/validate_classification.py scripts/tl_handoff.py scripts/tl_graft.py scripts/workflow_quality.py \
...
cp scripts/fixtures/workflow_quality/tasks.json "$export_dir/scripts/fixtures/workflow_quality/"
cp scripts/extract_tool_result.py scripts/context_ledger.py scripts/context_lib.py \
   scripts/tl_story_authority.py scripts/validate_execution_plan.py "$export_dir/scripts/"
printf '%s\n' "$export_dir"
...
Para conferir os 56 arquivos, execute a partir da mesma raiz:
...
shasum -a 256 README.md SKILL.md LICENSE \
  ...
  schemas/runtime-config.schema.json schemas/step-journal.schema.json \
  schemas/execution-plan.schema.json schemas/story-authority-envelope.schema.json \
  schemas/story-child-proposal.schema.json \
  ...
  docs/RUNTIME.md \
  scripts/tl_job.py scripts/tl_orc.py scripts/tl_runtime.py scripts/tl_ci_slice.py scripts/tl_tools.py scripts/validate_classification.py scripts/tl_handoff.py scripts/tl_graft.py \
  scripts/workflow_quality.py \
  scripts/validate_runtime_proof.py scripts/import_browser_proof.py \
  scripts/compare_skill_profiles.py scripts/validate_ambiguities.py \
  scripts/validate_gate_reuse.py scripts/preflight_workflow.py \
  scripts/fixtures/workflow_quality/tasks.json \
  scripts/extract_tool_result.py scripts/context_ledger.py scripts/context_lib.py \
  scripts/tl_story_authority.py scripts/validate_execution_plan.py \
  > "$checksum_file" &&
