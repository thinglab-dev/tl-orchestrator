...
seções `## Arquivos`, com SHA-256 e caminho relativo dos 54 arquivos; `## Integrações`, com
...
Os hashes de `## Arquivos` são gerados a partir dos 54 arquivos da revisão de origem já conferida
...
acompanhada, commit instalado, hashes dos 54 arquivos, destinos de skill, se cada destino é link
...
faz parte dos 54 arquivos distribuídos; não autoriza modelos nem precisa ser lida inteira por
...
apenas porque os 54 arquivos e hashes coincidem.
...
integração for versionada para a equipe, prefira uma cópia conferida dos 54 arquivos. Um link
...
carregado pertence a um destino registrado em `INSTALLATION.md` e compare os 54 arquivos com
...
3. prepare um plano que separe atualização dos 54 arquivos, migrações de `INSTALLATION.md` e
...
Depois das decisões, o Maker preserva modificações locais, instala os 54 arquivos de uma única
...
## Aceleração de contexto (opcional)

Quando o usuário pedir para ativar economia de contexto por código (por exemplo, "ative
economia de contexto"), use [`docs/GRAFT.md`](GRAFT.md) e `scripts/tl_graft.py`. É um
acelerador local e opcional: instala uma versão pinada do Graft oficial num cache isolado
por worktree, sem `graft init`, sem tocar `AGENTS.md`/`CLAUDE.md`/`GEMINI.md`/`.claude/`
nem qualquer configuração global de agente. O mapa gerado é um dado derivado para reduzir
exploração repetida — nunca autoridade de requisito ou revisão — e sua ausência, erro,
timeout ou desatualização nunca bloqueiam a tarefa nem substituem leitura direta/`rg`.
Ativá-lo não é pré-requisito do método.

## Runtime durável (opcional)

Quando um lote do Modo Automático já estiver autorizado e congelado e o usuário quiser que
ele rode sem sessão de Orquestrador aberta ("continue o épico enquanto eu durmo"), use
[`docs/RUNTIME.md`](RUNTIME.md) e `scripts/tl_runtime.py`. O consumidor mantém a
configuração em `_tl-orc/runtime.json` (adapters de harness, papéis, portões, limites,
caminhos sensíveis) validável por `schemas/runtime-config.schema.json`; a autoridade continua
vindo exclusivamente do lote (`permitted_effects`). O estado do runtime fica em
`_tl-orc/runtime/<lote>/` e não entra em commit. Ativá-lo não é pré-requisito do método.
