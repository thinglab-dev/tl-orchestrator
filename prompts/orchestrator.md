...
- **Higiene de worktrees:** qualquer worktree criado pelo método usa por padrão o pool oculto canônico `<repo-parent>/.worktrees/<repo-name>/<work-ref>`, derivado pelo supervisor; nunca crie irmãos visíveis `../wt-*` ou diretórios equivalentes ao lado dos projetos. Override de `pool_dir` exige configuração deliberada do consumidor. No fechamento, tente cleanup sem `--force`; se houver diff/untracked além do metadata do supervisor, erro de inspeção ou recusa do Git, preserve a árvore e mantenha a unidade bloqueada para reconciliação em vez de apagar trabalho.
