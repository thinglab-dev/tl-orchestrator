...
5. Para conduzir como Orquestrador, leia o [contrato](prompts/orchestrator.md) e o [playbook](prompts/orchestrator-playbook.md). Conduzir é coordenar e validar: pesquisa não trivial, planejamento, implementação, portões e auditoria completa do diff vão para os papéis próprios, enquanto conversa, decisão, briefing, consulta trivial, comando curto de coordenação e validação de evidências ficam com você, conforme [Coordenação e delegação](prompts/orchestrator.md#coordenação-e-delegação). Antes de qualquer despacho de papel classificado, consulte os [perfis](prompts/orchestrator-perfis.md): uma sessão econômica do [Classificador](prompts/classifier.md) escolhe modelo e effort dos papéis necessários na fase atual, usando somente o briefing e o recorte de [evidências de roteamento](docs/MODEL_ROUTING.md); valide o resultado antes de despachar, mesmo quando houver pins. O Orquestrador mantém a seleção do usuário no harness.
