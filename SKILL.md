...
3. Leia as instruções aplicáveis ao consumidor, a tarefa atual e seu estado real, usando o [guia de descoberta](docs/PROJECT_CONFIGURATION.md) para localizar regras, portões e evidências. Resolva todo caminho `_tl-orc/...` a partir da raiz do projeto consumidor.
   ...
   - **Handoff remoto opt-in:** quando `remote_handoff: enabled` estiver registrado nas preferências operacionais e o preflight estritamente local `python <pacote>/scripts/tl_handoff.py availability --remote-enabled` devolver `remote_available: true`, ofereça **ChatGPT + RDC** e **Continuar localmente**. O preflight executa somente `command -v codex` e `codex --version`; não reserva budget nem chama modelo. Se falhar, mantenha a opção local e exponha o motivo, sem fallback silencioso. A escolha remota segue o protocolo fechado em [handoff remoto](docs/WORK_MODEL.md#handoff-remoto-chatgpt--rdc); fora dela, o modo local é compatível por omissão.
