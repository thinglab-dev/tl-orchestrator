...
- Story Authority Envelope e lotes filhos autônomos — `AUTO_STORY` (Task Native T032):
  ...
  - **Vínculo do lote executável à derivação registrada**: o evento `child_derived` carrega a `child_proposal` e a `derivation_proof` inteiras num único append atômico, e é a fonte de verdade; os digests declarados pelo lote são alegações. `record_child_derived()` recalcula os digests, re-prova envelope, linhagem e cadeia e nunca substitui um filho já registrado (registro idêntico é replay idempotente). Antes de qualquer worker, sob o lease, `bind_child_batch()` recupera os documentos registrados para exatamente `batch.id`, recalcula-os, compara com o evento e com o lote, re-prova tudo e exige que o lote não peça nada além do filho (`work_ref`, spec, `scope_paths`, efeitos — `pull_request_merge` ↔ `merge` —, orçamento e checkpoint). Lote não derivado, digest adulterado, documento editado no journal, `parent_child_batch_id` inexistente, pai não fechado, pai `approved`, bifurcação de linhagem e segundo filho que se apresenta como primeiro são `HARD STOP` sem batch journal aberto e sem reserva de chamada. `verify_derivation`, `record_child_derived` e `bind_child_batch` executam os mesmos geradores de prova, sem segunda implementação no runtime.
  - **Forbidden aninhado em escopo amplo é política executável**: `forbidden_paths` e os padrões de `protected_paths` do filho vinculado tornam-se `do_not_touch` de cada unidade — presentes no Context Pack antes do Maker e impostos pela contenção pós-Maker, que recusa a rodada e restaura a árvore para criação, alteração, deleção ou rename de qualquer path coberto, mesmo quando `scope_paths` é um diretório que o contém. Um `scope_path` dentro de um path proibido ou protegido é recusado no vínculo.
  - **Findings residuais ligados ao que o Checker declarou**: `unresolved_action_items_digest` passa a cobrir `scope_status`, `spec_status` e `derivation_blockers`, de modo que remover um bloqueio no caminho até a proposta muda o digest registrado no fechamento do pai.
...
- Validador de Plano de Execução e Protected Paths de primeira classe (Task Native T033):
  ...
  - **Satisfatibilidade aritmética do orçamento**: `gate_call_constraints_are_satisfiable_under_budget` exige a declaração dos cenários `straight_line` e `retry` e prova que ambos cabem no teto de chamadas. `straight_line` tem exatamente zero `rework_rounds` e `retry` ao menos um; nomes de cenário são únicos e uma rodada precisa custar ao menos uma chamada — `retry` com zero rodadas, nome duplicado ou rodada gratuita são recusados mesmo quando a aritmética "fecha".
  ...
  - **Validador de schema que recusa o que não aplica**: subconjunto estrito de Draft 2020-12 em biblioteca padrão que refuta qualquer keyword não implementada em vez de ignorá-la, e que aplica `pattern` com a semântica ECMA-262 (`$` é fim da entrada, então valor seguido de quebra de linha não casa `^...$`); `tests/test_execution_plan_validator.py` prova que todos os schemas do repositório são integralmente enforceáveis por ele.
