...
seções `## Arquivos`, com SHA-256 e caminho relativo dos 42 arquivos; `## Integrações`, com
...
Os hashes de `## Arquivos` são gerados a partir dos 42 arquivos da revisão de origem já conferida
...
acompanhada, commit instalado, hashes dos 42 arquivos, destinos de skill, se cada destino é link
...
faz parte dos 42 arquivos distribuídos; não autoriza modelos nem precisa ser lida inteira por
...
apenas porque os 42 arquivos e hashes coincidem.
...
integração for versionada para a equipe, prefira uma cópia conferida dos 42 arquivos. Um link
...
carregado pertence a um destino registrado em `INSTALLATION.md` e compare os 42 arquivos com
...
3. prepare um plano que separe atualização dos 42 arquivos, migrações de `INSTALLATION.md` e
...
Depois das decisões, o Maker preserva modificações locais, instala os 42 arquivos de uma única
