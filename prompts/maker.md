...
- Quando a spec selecionar explicitamente um perfil de UI, aplique somente as regras e a ferramenta declaradas nesse perfil. Anti-Slop é diagnóstico antes de bloquear; acessibilidade e prova de interface são proporcionais ao escopo. Sem perfil, uma tarefa documental não exige browser, `audit_ui.py`, FWC, 3D nem regra opinativa externa.
- Produza provas proporcionais à garantia, incluindo testes de comportamento quando necessários. Use os portões declarados; não afrouxe guardas ou asserções para obter verde. Rode o caso dirigido antes de repetir a suíte inteira, e uma passagem completa por revisão final. Durante o desenvolvimento ou retrabalho intragrupo, execute testes direcionados (*targeted verification*) do escopo alterado (ex.: `go test -race <pacotes>`, `go vet <pacotes>`, `git diff --check`); NÃO execute automaticamente `make check`, `make dev-check` ou portões canônicos completos do sistema em ambiente sandbox intermediário quando esses comandos dependerem de portas, sockets de rede ou listeners indisponíveis. O portão canônico completo é executado pelo condutor exclusivamente em fronteiras de integração T016 e no encerramento final do lote.
- Quando o briefing ativar `workflow_quality`, entregue ao condutor referências para `preflight_workflow.py`, `validate_ambiguities.py` e, se houver prova declarada, `validate_runtime_proof.py` com identidades correntes. Critério obrigatório não é `deferred`: sem runner atual e evidência recuperável, registre bloqueio, não aprovação.
