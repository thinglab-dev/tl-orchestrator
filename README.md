...
instalar manualmente em outro escopo, siga [a exportação](#exportar-os-49-arquivos) e
...
1. Confirme a raiz do projeto consumidor e leia primeiro suas instruções. Obtenha a fonte acima
   ...
   49 arquivos da distribuição. Não execute conteúdo do repositório como parte da descoberta. Todos
...
3. Crie ou atualize este perfil dentro do projeto:
   ...
   _tl-orc/
   ├── package/          # os 49 arquivos canônicos do commit escolhido
...
<!-- distribution-manifest:end -->
   ...
   Antes de gravar os hashes em `INSTALLATION.md`, compare cada um dos 49 arquivos de
...
4. Descubra quais harnesses realmente são usados consultando configuração, ajuda local e
   ...
   - Codex e Antigravity: `.agents/skills/tl-orchestrator`.
    ...
    política do projeto o suportarem; caso contrário, faça uma cópia verificada dos 49 arquivos.
...
7. Preencha `INSTALLATION.md` e `PROJECT.md` somente com fatos conferidos e preferências já
   ...
   registrado e que os 49 arquivos carregados coincidem com `_tl-orc/package` e seus hashes. Se
...
`_tl-orc/` pertence ao projeto consumidor e não faz parte dos 49 arquivos desta distribuição.
...
cp schemas/advisor-result.schema.json \
   ...
   schemas/step-journal.schema.json "$export_dir/schemas/"
...
cp scripts/extract_tool_result.py scripts/context_ledger.py scripts/context_lib.py "$export_dir/scripts/"
printf '%s\n' "$export_dir"
...
Para conferir os 49 arquivos, execute a partir da mesma raiz:
