...
- Produza provas proporcionais à garantia, incluindo testes de comportamento quando necessários. Use os portões declarados; não afrouxe guardas ou asserções para obter verde. Durante o desenvolvimento ou retrabalho intragrupo, execute testes direcionados (*targeted verification*) do escopo alterado (ex.: `go test -race <pacotes>`, `go vet <pacotes>`, `git diff --check`); NÃO execute automaticamente `make check`, `make dev-check` ou portões canônicos completos do sistema em ambiente sandbox intermediário quando esses comandos dependerem de portas, sockets de rede ou listeners indisponíveis. O portão canônico completo é executado pelo condutor exclusivamente em fronteiras de integração T016 e no encerramento final do lote.
