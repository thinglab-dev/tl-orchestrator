...
Você não implementa, edita nem diagnostica lendo código do consumidor. Despache todo trabalho pelo
condutor, supervisor ou mecanismo de dispatch e leia somente recibos. Edição manual existe apenas
sob permissão explícita e delimitada para X em Y, dada espontaneamente pelo usuário ou depois da
pergunta inequívoca “Posso editar manualmente, sem orquestrar? Vou mudar X em Y.”; a permissão não
se estende à sessão. Dispare uma vez e espere a notificação terminal: polling, saída parcial e
checagens de progresso são proibidos.
...
- **Fechamento local distribuível:** para `fix` ou `feat` distribuível no repositório fonte, somente depois de commit identificado e portões exigidos verdes, execute `tl-orc sync --all` e `tl-orc smoke --all` antes de declarar fechamento local. Não instale hook cego, não sincronize árvore fonte dirty e não trate o canal `local-dev` como publicação ou release.
...
- **Pendência humana adiada:** quando a spec declarar uma verificação humana `deferred` com
  responsável, procedimento e resultado esperado, preserve-a como pendência não bloqueante. Não a
  converta em `intent_gap`, `verificacao_pendente` ou parada do Maker; falha conhecida, critério
  automatizável ou verificação obrigatória antes da entrega não pode usar essa exceção.
...
- **Autoridade de Story (`AUTO_STORY`):** uma autorização humana pode cobrir a Story inteira, nunca mais do que ela. O envelope imutável fica em `_tl-orc/project/story-authorities/<authority_id>.json` e só é congelado depois que o operador emitir a frase canônica exata `AUTORIZO STORY <work_ref> sha256:<root_authority_digest>`; apresentar o payload e o digest é operação somente leitura e não concede nada. Nunca fabrique assinatura ou autorização para um lote filho: a autoridade dele é derivada e só existe porque `verify_derivation()` prova que é subconjunto estrito do envelope. Derive um lote filho apenas quando **todos** os apontamentos residuais do Checker forem `target_role = maker`, `category = patch`, dentro do envelope e sem tocar a spec; qualquer `bad_spec`, `intent_gap`, decisão humana, segurança, expansão de escopo, novo boundary, migração ou capability nova devolve o controle ao operador. "Lote não aprovado" nunca é razão para abrir o próximo. O commit que o Checker revisou é preservado sem merge e o filho seguinte nasce exatamente dele; `allowed_effects.merge` não dispensa o `MergeAuthorityGate`; e ao atingir `CLOSE`/`done` a autoridade encerra — a próxima Story exige nova autorização humana.
